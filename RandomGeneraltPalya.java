...
public class RandomGeneraltPalya implements Palya{
	...
	KezdetiParams kezdetiParams = new KezdetiParams(2);
	...
	public void aKomplettPalyaLeGeneralasa() {
		/**
		 * seged tarolok letrehozasa
		 */
		
		String tipus[] = {"D", "D", "D", "M", "M", "M", "M", "H", "H", "H",
				"R", "R", "R", "E", "E", "E"};
		String szamok[] = {"02", "03", "04", "04", "05", "05", "06", "06", "08", "08",
				"09", "09", "10", "10", "11", "12"};
		
		List<String> tipusokLista = new ArrayList<String>();
		List<String> szamokLista = new ArrayList<String>();
		...
		for( int i = 0; i < 16; ++i) {
			tipusokLista.add(tipus[i]);
			szamokLista.add(szamok[i]);
		}

		int nyersMezokI, nyersMezokJ, randomIndex, randomSzamTipusra, randomSzamSzamokra;
		//inic, hogy "engedje" lejjebb osszefuzni oket
		String tipusBe = "";
		String szamBe = "";
		String teljesBe = "";
		randomIndex = 16;
		for( int i = 0; i < 16; ++i ) {
			//random szamokat korrigalom
			randomSzamTipusra = random.nextInt(randomIndex);
			randomSzamSzamokra = random.nextInt(randomIndex);
			randomIndex -= 1;
			...
			//random altal generalt indexet kinyerem, majd kitorlom
			for(int j = 0; j < tipusokLista.size(); ++j ) {
				if( j == randomSzamTipusra) {
					tipusBe = (tipusokLista.get(j));
					tipusokLista.remove(j);
				}
			}
			for(int j = 0; j < szamokLista.size(); ++j ) {
				if( j == randomSzamSzamokra) {
					szamBe = (szamokLista.get(j));
					szamokLista.remove(j);
				}
			}
			
			//osszefuzom a kapott mezo darabjait
			teljesBe = (tipusBe + szamBe);
			
			//beirom a mezore
			nyersanyagMezok[nyersMezokI][nyersMezokJ] = teljesBe;
		}
	...
	public String dobas() {
		...
		for(int i = 0; i < nyersanyagMezok.length; ++i) {
			for(int j = 0; j < nyersanyagMezok.length; ++j) {
	...
	public int mezoSzamKinyero(int oszlop, int sor) {
		return Integer.parseInt( String.valueOf(nyersanyagMezok[sor][oszlop].charAt(1)) +
				String.valueOf(nyersanyagMezok[sor][oszlop].charAt(2)) );
	}
	
	@Override
	public String mezoTipusKinyero(int oszlop, int sor) {
		return String.valueOf(nyersanyagMezok[sor][oszlop].charAt(0));
	}
